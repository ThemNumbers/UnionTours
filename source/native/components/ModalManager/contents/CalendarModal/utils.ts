...
import { formatDate, isBetweenDates } from '../../../../utils/formatDates'
import { capitalizeFirstLetter } from '../../../../utils/textHelper'

export type ListType = 'year' | 'month' | 'day'
...
export const monthFormatter = (value: number) =>
  String(capitalizeFirstLetter(formatDate(new Date(2022, value, 5), 'LLLL')))

export const yearFormatter = (value: number) => String(value)
...
export const getYearValues = (min: number, max: number) => {
  return new Array(max - min + 1).fill(0).map((_, i) => min + i)
}

export const getMonthsValues = () => {
  return new Array(12).fill(0).map((_, i) => i)
}
